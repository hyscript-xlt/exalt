...
public class Client {
    ...
    public static Client newInstance() {
        return Try.of(() -> new Client(AerospikeClientFactory.ofNewInstance().get()))
                .get();
    }
    ...
    public List<KeyRecord> findByLongValue(String setName, String key, long value) {
        ...
        return findBy(setName, Filter.equal(key, value));
    }
    ...
    private void createIndex(String setName, String key, IndexType indexType) {
        String host = Try.of(ApplicationConfig::getDbHost).getOrElse("");
        int port = Try.of(ApplicationConfig::getDbPort).getOrElse(0);
        if (!Info.request(host, port, "sindex").contains(key)) {
    ...
    private List<KeyRecord> findBy(String setName, Filter filter) {
        ...
        return Optional.ofNullable(client.query(null, stmt))
                .map(r -> Try.of(r::iterator).getOrElse(Collections.emptyIterator()))
    ...
    /**
     * Dedicated method for default DB name, all methods in the class should refer this method<br/>
     * if there is a need to connect to the default DB.
     */
    private String getDbName() {
        return Try.of(ApplicationConfig::getDbName)
